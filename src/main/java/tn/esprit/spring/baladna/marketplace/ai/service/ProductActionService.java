...
public class ProductActionService {
    ...
    public Map<String, Object> executeAction(String aiResponse, Integer artisanId) {
        ...
        try {
            ...
            return switch (action) {
                ...
                case "DELETE_PRODUCT_BY_NAME"  -> deleteProductByName(json, artisanId);
    ...
    private Map<String, Object> addProduct(JsonNode json, Integer artisanId) {
        ...
        try {
            ...
            jdbcTemplate.update(sql, name, price, stock, artisanId, idCategorie,
                    "Added via Baladna AI Assistant", imageUrl, now, now);
    ...
    private Map<String, Object> deleteProduct(JsonNode json, Integer artisanId) {
        ...
        try {
            ...
            int rows = jdbcTemplate.update("DELETE FROM product WHERE id_produit = ? AND id_artisan = ?", productId, artisanId);
            if (rows > 0) {
                result.put("success", true);
    ...
    // ==================================================================
    private Map<String, Object> deleteProductByName(JsonNode json, Integer artisanId) {
        Map<String, Object> result = new HashMap<>();
        try {
            String name = json.get("productName").asText();
            int rows = jdbcTemplate.update(
                    "DELETE FROM product WHERE LOWER(nom_produit) = LOWER(?) AND id_artisan = ?", name, artisanId);
            if (rows > 0) {
                result.put("success", true);
                result.put("message", "✅ Deleted: " + name);
            } else {
                result.put("success", false);
                result.put("message", "'" + name + "' not found.");
            }
        } catch (Exception e) {
            result.put("success", false);
        ...
        try {
            List<Map<String, Object>> products = jdbcTemplate.queryForList(
                    "SELECT id_produit, nom_produit, prix_produit, stock_produit, date_creation " +
                            "FROM product WHERE id_artisan = ? ORDER BY date_creation DESC LIMIT 20", artisanId);
            StringBuilder sb = new StringBuilder("📦 Your products:\n");
            for (Map<String, Object> p : products) {
                sb.append(String.format("• #%s - %s - %.2f TND (stock: %s)\n",
                        p.get("id_produit"), p.get("nom_produit"), p.get("prix_produit"), p.get("stock_produit")));
            }
            result.put("success", true);
        ...
        try {
            List<Map<String, Object>> orders = jdbcTemplate.queryForList(
                    "SELECT c.* FROM commandes c " +
                            ...
                            "WHERE p.id_artisan = ? ORDER BY c.created_at DESC LIMIT 10", artisanId);
            result.put("success", true);
    ...
    private Map<String, Object> getStats(Integer artisanId) {
        ...
        try {
            ...
            Double stockValue = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(prix_produit * stock_produit), 0) FROM product WHERE id_artisan = ?", Double.class, artisanId);
            Double avgStock = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(AVG(stock_produit), 0) FROM product WHERE id_artisan = ?", Double.class, artisanId);
            result.put("success", true);
